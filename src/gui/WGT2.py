...
class GNode(pygame.sprite.Sprite):
    ...
    def sndrcvpacket(self, packet=None, rcv=False, snd=False):
        ...
        if rcv == True:
            ...
            #    if not self.queue.full == True:
                # self.queue.add(packet.color) 
                # Adds this packet to the queue to route
        ...
        if snd == True:
            ...
            self.ctr.fill(colors['CKEY'])
            self.ctr.set_colorkey(colors['CKEY'])
            #if not self.queue.empty == True:
                #self.ctr = Gdraw_bkg(type='packet', color=self.queue[0], self.bg.get_width())
                #self.ctrovr = Gload_image('packet_overlay.png')
                #self.ctrovr, self.ctr = Gequalize_images(self.ctrovr, self.bg, self.ctr)
                #self.ctr.blit(self.ctrovr, (0, 0))
                #self.queue.remove()
